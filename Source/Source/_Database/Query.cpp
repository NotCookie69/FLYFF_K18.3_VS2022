...
CQuery::CQuery()
{
	...
	for (int i = 0; i < MAXCOL; i++) //__INVENTORY_168
...
*/
	...
	while( TRUE )
	{
		...
		{
			if( szSQL )
				WriteLogFile("query:%s\nSQLSTATE:%s error:%s\n//================================================================================================================", szSQL, (LPCTSTR)szState,(LPCTSTR)szMsg );
			else
				WriteLogFile("SQLSTATE:%s error:%s\n//================================================================================================================", (LPCTSTR)szState,(LPCTSTR)szMsg );
			nIndex++;
		}
		else
			break;	
...
BOOL CQuery::PrepareFetch()
{
	...
	{
		SQLBindCol(hStmt, c + 1, SQL_C_CHAR, Col[c], 51000, &lCol[c]); //8192 __INVENTORY_168
