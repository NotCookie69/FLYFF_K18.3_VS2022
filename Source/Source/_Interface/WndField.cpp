...
#ifdef __LOOKCHANGE
			itemElem.SetLook( pMover->m_aEquipInfo[i].dwLook );
#endif
...
#ifdef __LOOKCHANGE
			DWORD dwId;
			if( pMover->m_aEquipInfo[PARTS_HAT].dwLook != NULL_ID )
				dwId	= pMover->m_aEquipInfo[PARTS_HAT].dwLook;
			else
				dwId = pMover->m_aEquipInfo[PARTS_CAP].dwId;
#else
...
void CWndInventory::OnDraw(C2DRender* p2DRender)
{
	...
	CWndButton* pSort = (CWndButton*)GetDlgItem(WIDC_BUTTON1);
	if (pSort)
	{
		pSort->SetTexture(D3DDEVICE, MakePath(DIR_THEME, "ButtSortInv.tga"));
	}
#endif
...
*/		
	...
	if( g_pPlayer )
		...
		if( pItemElem )
		{
			O3D_ELEMENT* pElement = NULL;
#ifdef __LOOKCHANGE
			ItemProp* pItemProp = pItemElem->GetLook() != NULL_ID ? prj.GetItemProp( pItemElem->GetLook() ) : pItemElem->GetProp();
#else
			...
			if( pItemElemOvercoat )
			{
				...
				{
#ifdef __LOOKCHANGE
					ItemProp* pItemPropOC = pItemElemOvercoat->GetLook() != NULL_ID ? prj.GetItemProp( pItemElemOvercoat->GetLook() ) : pItemElemOvercoat->GetProp();
#else
...
#ifdef __LOOKCHANGE
					if( g_WndMng.GetWndBase( APP_LOOKCHANGE ))
					{
						if( pFocusItem->GetExtra() < ( (CItemElem*)pFocusItem )->m_nItemNum )
						{	
							CWndLookChange* pWndLookChange = (CWndLookChange*)GetWndBase( APP_LOOKCHANGE );
							pWndLookChange->SetItem( (CItemElem*)pFocusItem );
							return TRUE;
						}
					}
#endif
...
#endif // __RESTATE_CONFIRM
#ifdef __RANDOMOPTION_RETRY
					if( pFocusItem->m_dwItemId == II_SYS_SYS_SCR_RANDOMOPTIONRTRY )
					{
...
//							break;
	...
	if( message == WNM_CLICKED )
	{
		...
		case WIDC_BUTTON1:
		{
			g_DPlay.SendHdr(PACKETTYPE_SORT_INVENTORY);
		}
		break;
#endif
			//case 101: // report
...
void CWndInventory::RunUpgrade( CItemBase* pItem )
{
	if( m_bIsUpgradeMode )
	{
		...
		else if( IsNeedTarget( pItemProp ) )
		{
			...
			m_dwEnchantWaitTime		= g_tmCurrent + SEC(ENCHANT_TIME);
			return;
		}
#endif	// __VER
#if __VER >= 9 // __CSC_VER9_1 //오리칼쿰, 문스톤 얼터멋 웨폰 제련 관련 변경.
...
{ 
	if( nID == WIDC_YES )
	{
		if(m_pInventory != NULL)
		{
			...
			m_pInventory->m_dwEnchantWaitTime = g_tmCurrent + SEC(ENCHANT_TIME);
		}
		Destroy();
	}
	else if( nID == WIDC_NO || nID == WTBID_CLOSE )
	...
	case WND_NORMAL:
		{
			if( m_pItemElem->GetAbilityOption() < GENERAL_NON_USING_SCROLL2_LEVEL && m_pItemElem->GetProp()->dwReferStat1 != WEAPON_ULTIMATE )
			{
				if( pItemProp->dwID == II_SYS_SYS_SCR_SMELTING
					...
					|| pItemProp->dwID == II_SYS_SYS_SCR_NOR_SMELTING01
#endif
#ifdef __SYS_SCR_NOR_SMELTING02
					|| pItemProp->dwID == II_SYS_SYS_SCR_NOR_SMELTING02
#endif
#ifdef __SYS_SCR_NOR_SMELTING03
					|| pItemProp->dwID == II_SYS_SYS_SCR_NOR_SMELTING03
#endif
					)
					bAcceptableItem = TRUE;
			}
...
BOOL CWndLookChange::SetItem(CItemElem *pItem)
{
	...
	if (m_pItemDest)
	{
		...
		if (nMsg == -1)
		{
			...
			{
				nMsg = TID_MMI_ELLDINPOTION_TEXT08;
			}
		}
		else
...
#ifdef __RANDOMOPTION_RETRY
CWndRandomOptionRetry::CWndRandomOptionRetry()
{
	m_pItemElem = NULL;
...
CWndRandomOptionRetry::~CWndRandomOptionRetry()
{
	...
	if (m_pMaterialItem)
		m_pMaterialItem->SetExtra(0);
}

void CWndRandomOptionRetry::OnDraw(C2DRender* p2DRender)
{
	if (m_pItemElem && m_pItemElem->GetTexture())
	{
		...
		m_pItemElem->GetTexture()->Render(p2DRender, point, 255);

		CRect hitrect = m_Rect;
		CPoint point2 = GetMousePoint();

		if (m_Rect.PtInRect(point2))
		{
			CPoint point3 = point2;
			ClientToScreen(&point3);
			ClientToScreen(&hitrect);

			g_WndMng.PutToolTip_Item((CItemBase*)m_pItemElem, point3, &hitrect);
		}
	}
	...
	CWndButton* pWndButton4 = (CWndButton*)GetDlgItem(WIDC_START);
	...
	if (m_pItemElem)
	{
		...
		{
			if (g_xRandomOptionProperty->GetParam(m_pItemElem->GetRandomOptItemId(), 0, &nDst, &nAdj))
			{
				if (nAdj > 0)
					nChar = '+';
				else
				...
				{
					if (nDst == DST_ATTACKSPEED)
						str.Format("%s %c%d%%", FindDstString(nDst), nChar, ::abs(nAdj / 2 / 10));
					else
						str.Format("%s %c%d%%", FindDstString(nDst), nChar, ::abs(nAdj));
				}
				else
					str.Format("%s %c%d", FindDstString(nDst), nChar, ::abs(nAdj));

				pWndButton->SetTitle(str);
			}
		}

		if (pWndButton2->IsWindowEnabled())
		{
			if (g_xRandomOptionProperty->GetParam(m_pItemElem->GetRandomOptItemId(), 1, &nDst, &nAdj))
			{
				if (nAdj > 0)
					nChar = '+';
				else
				...
				pWndButton2->SetTitle(str);
			}
		}

		if (pWndButton3->IsWindowEnabled())
		{
			if (g_xRandomOptionProperty->GetParam(m_pItemElem->GetRandomOptItemId(), 2, &nDst, &nAdj))
			{
				if (nAdj > 0)
					nChar = '+';
				else
				...
				pWndButton3->SetTitle(str);
			}
		}

		if (pWndButton->GetCheck() || pWndButton2->GetCheck() || pWndButton3->GetCheck())
			pWndButton4->EnableWindow(TRUE);
	}
}

BOOL CWndRandomOptionRetry::OnDropIcon(LPSHORTCUT pShortcut, CPoint point)
{
	CWndBase* pWndFrame = pShortcut->m_pFromWnd->GetFrameWnd();
	...
	if (PtInRect(m_Rect, point))
	{
		...
		{
			g_WndMng.OpenMessageBox(prj.GetText(TID_MMI_RANDOMOPTIONRETRY_ERRORTEXT03));
		}
	}
...
BOOL CWndRandomOptionRetry::Initialize(CWndBase* pWndParent, DWORD nType)
{
	return CWndNeuz::InitDialog(g_Neuz.GetSafeHwnd(), APP_RANDOMOPTIONRETRY, 0, 0, pWndParent);
}
...
BOOL CWndRandomOptionRetry::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	...
	CItemElem* pItemElem = g_pPlayer->m_Inventory.GetAtId(II_SYS_SYS_SCR_RANDOMOPTIONRTRY);
...
void CWndRandomOptionRetry::OnInitialUpdate()
{
	CWndNeuz::OnInitialUpdate();
	...
	LPWNDCTRL pCustom = NULL;
	pCustom = GetWndCtrl(WIDC_PICTURE1);
	m_Rect = pCustom->rect;
...
void CWndRandomOptionRetry::OnRButtonUp(UINT nFlags, CPoint point)
{
	if (m_pItemElem && PtInRect(m_Rect, point))
	{
		m_pItemElem->SetExtra(0);
		m_pItemElem = NULL;
		CWndButton* pWndButton = (CWndButton*)GetDlgItem(WIDC_RADIO1);
		pWndButton->EnableWindow(FALSE);
		pWndButton->SetTitle("Radio Button");
		pWndButton->SetCheck(FALSE);
		CWndButton* pWndButton2 = (CWndButton*)GetDlgItem(WIDC_RADIO2);
		pWndButton2->EnableWindow(FALSE);
		pWndButton2->SetTitle("Radio Button");
		pWndButton2->SetCheck(FALSE);
		CWndButton* pWndButton3 = (CWndButton*)GetDlgItem(WIDC_RADIO3);
		pWndButton3->EnableWindow(FALSE);
		pWndButton3->SetTitle("Radio Button");
		pWndButton3->SetCheck(FALSE);
		CWndButton* pWndButton4 = (CWndButton*)GetDlgItem(WIDC_START);
		pWndButton4->EnableWindow(FALSE);
	}
}

void CWndRandomOptionRetry::SetItemId(CItemElem* pItemElem)
{
	pItemElem->SetExtra(pItemElem->GetExtra() + 1);
	m_pMaterialItem = pItemElem;
}
#endif