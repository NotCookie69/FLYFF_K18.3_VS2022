		...
		for( unsigned i = nPos; i < nEnd; ++i )
		{
			...
			for(unsigned j = 0; j < pSet->vecSet[i].vecCondItem.size(); j++)
			{
				...
				if( pPropItem )
				{
					...
					m_p2DRender->TextOut( pt.x + 29 - size.cx, pt.y + 31 - size.cy + 4, szTemp, 0xFF07A878);
					p2DRender->SetFont( pOldFont );
			...
			for(unsigned j = 0; j < pSet->vecSet[i].vecPayItem.size(); j++)
			{
				...
				if( pPropItem )
				{
					...
					m_p2DRender->TextOut( pt.x + 29 - size.cx, pt.y + 29 - size.cy + 4, szTemp, 0xFF07A878);
					p2DRender->SetFont( pOldFont );
			...
			p2DRender->RenderLine(CPoint(0, pt.y), CPoint(GetWindowRect().Width() - 10, pt.y), 0xFFB5BEBE);
		}
...
int CWndComboBox::AddString(LPCTSTR lpszString)
{
	...
	rect.bottom = rect.top + (((nNum > 8 ? 8 : nNum)) * (m_pFont->GetMaxHeight() + 3)) + 8;
