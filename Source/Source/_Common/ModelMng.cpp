...
CModel* CModelMng::LoadModel( LPDIRECT3DDEVICE9 pd3dDevice, int nType, int nIndex, BOOL bParts )
{
	...
	if( lpModelElem == NULL ) 
	{
		Error( "CModelMng::loadModel objtype=%d index=%d bpart=%d", nType, nIndex, bParts );
		return NULL;
	}
...
BOOL CModelMng::LoadScript( LPCTSTR lpszFileName )
{
	...
	while( script.tok != FINISHED )
	{
		...
		while( nBrace )
		{
			...
			{
				CString str;
				str.Format( "CModelMng::LoadScript( Line : %d) : Object : %s, %s", script.GetLineNum(), szObject, script.token );
				AfxMessageBox( str );
			}
			...
			if( *script.token == '{' )
			{
				...
				while( *script.token != '}' )
				{
				#ifdef _DEBUG
					if( sizeof(szMotion) <= strlen(script.token) + 1 )
						Error( "File : %s %d", lpszFileName, strlen(script.token) );
				#endif
					_tcscpy( szMotion, script.token );
					iMotion = script.GetNumber();
					...
					{
						CString str;
						str.Format( "CModelMng::LoadScript(Line :%d) File : %s Motion : %s", script.GetLineNum(), lpszFileName, lpszMotion );
						AfxMessageBox( str );
					}
			...
			{
				CString str;
				str.Format( "CModelMng::LoadScript(Line : %d)Files : %s : type = %d, idx = %d, name = %s", script.GetLineNum(), lpszFileName, iType, iObject, modelElem.m_szName );
				AfxMessageBox( str );
			}
