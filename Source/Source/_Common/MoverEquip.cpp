	...
	for( int i = 0; i < MAX_HUMAN_PARTS; i++ )
	{
		...
		if( pInventory == NULL ) // fake only
		{
			if( pEquipInfo[i].dwId != NULL_ID ) 
			{
				...
				{
#ifdef __LOOKCHANGE
					if( pEquipInfo[i].dwLook != NULL_ID )
						pItemProp = prj.GetItemProp( pEquipInfo[i].dwLook );
					else
#endif
					{
					  ...
					  PEQUIP_INFO pEquipInfo, CModelObject* pModel, BOOL bEquip, CMover *pMover ) 
{
	...
	if( bEquip )
	{
		...
		if( dwLook != NULL_ID )
		{
			...
			if( pProp )
			{
				if( pProp->bPartsFile == TRUE )
					prj.m_modelMng.MakePartsName( szPartsName, _T( "part" ), pProp->dwID, nSex );
				else
