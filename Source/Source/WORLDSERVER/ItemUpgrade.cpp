...
BYTE	CItemUpgrade::SmeltSafetyGeneral( CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial, CItemElem* pItemProtScr, CItemElem* pItemSmeltScr )
{
	...
	if( pItemSmeltScr != NULL )
	{
		...
		else if (IsUsableItem(pItemSmeltScr) && pItemSmeltScr->GetProp()->dwID == II_SYS_SYS_SCR_NOR_SMELTING01)
		{
			if (pItemMain->GetAbilityOption() >= 3 && pItemMain->GetAbilityOption() < 7)
			{
				nPercent += 1000;
				ItemProp* pItemProp = pItemSmeltScr->GetProp();
				if (pItemProp)
		...
		else if (IsUsableItem(pItemSmeltScr) && pItemSmeltScr->GetProp()->dwID == II_SYS_SYS_SCR_NOR_SMELTING02)
		{
			if (pItemMain->GetAbilityOption() >= 7 && pItemMain->GetAbilityOption() < 9)
			{
				nPercent += 500;
				ItemProp* pItemProp = pItemSmeltScr->GetProp();
				if (pItemProp)
		...
		else if (IsUsableItem(pItemSmeltScr) && pItemSmeltScr->GetProp()->dwID == II_SYS_SYS_SCR_NOR_SMELTING03)
		{
			if (pItemMain->GetAbilityOption() == 9)
			{
				nPercent += 300;
				ItemProp* pItemProp = pItemSmeltScr->GetProp();
				if (pItemProp)
